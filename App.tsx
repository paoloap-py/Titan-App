...
const ReportsView: React.FC<{ sessions: WorkoutSession[]; prs: Record<string, ExercisePR[]>; protocol: Protocol }> = ({ sessions, protocol }) => {
...
const BodyStatsView: React.FC<{ 
  ...
  accentColor: string;
}> = ({ bodyWeights, onAddWeight, onDeleteWeight, accentColor }) => (
  <div className="space-y-8 animate-in fade-in duration-500">
...
const App: React.FC = () => {
  ...
  const [activeProtocolId, setActiveProtocolId] = useState('titan-133');
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-gray-200 pb-24 lg:pb-0 lg:pl-72 flex flex-col">
      ...
      <main className="max-w-6xl w-full mx-auto p-4 lg:p-12">
        ...
        {activeTab === 'reports' && <ReportsView sessions={sessions} prs={{}} protocol={activeProtocol} />}
        {activeTab === 'body' && (
          <BodyStatsView 
            bodyWeights={bodyWeights} 
            onAddWeight={(w) => setBodyWeights(prev => [w, ...prev])} 
            onDeleteWeight={(id) => setBodyWeights(prev => prev.filter(w => w.id !== id))} 
            accentColor={activeProtocol.accentColor} 
          />
        )}
